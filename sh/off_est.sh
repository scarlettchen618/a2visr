...
if [ $? != 0 ]; then
  ...
  tmux send-keys -t $SESSION_NAME:0.0 'roslaunch mavros px4.launch' 
  tmux send-keys -t $SESSION_NAME:0.1 'roslaunch serial_imu trans_est_node.launch'
  tmux send-keys -t $SESSION_NAME:0.2 'roslaunch vrpn_client_ros sample.launch'
  tmux send-keys -t $SESSION_NAME:0.3 'roslaunch nlink_parser linktrack.launch'
fi
